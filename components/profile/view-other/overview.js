...
const Overview = ({ data, ownUserId, callbackSetDataOption, callbackLoadData, hasMoreData, deletePost }) => {
        ...
        <Box sx={{ px: 1 }}>
          {data.map((record, index) => (
            <Paper key={index} sx={{ mb: 3, p: 1 }} elevation={3}>
              <AbstractPost deletePost={deletePost} data={record} ownUserId={ownUserId} />
            </Paper>
          ))}
        </Box>
      </InfiniteScroll>
